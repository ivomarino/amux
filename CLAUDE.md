...
| `GET /api/health/invariants` | The check that CAN fail on the above: `route.callers_have_routes` enumerates SPA/CLI call sites against the mounted table and names each miss (404 vs "route exists but allows only \[POST\]"). Read `failures` and match on `invariant_id` — there is no `results` key, and rows carry `invariant_id`, not `id`. **A PASSING invariant appears nowhere here** — `checks` is a bare count, so an invariant that is green and one that was never wired in look identical. To confirm a specific check RAN, use the next row. |
| `GET /api/debug/invariants` | `latest_per_invariant` names every invariant with its `status` and `age_s` — the only place a PASS is visible, and therefore the only way to tell a green check from an absent one after you add one. `stale_checks` lists checks with no evaluation in >300s, which is worse than a failing check because silence reads as health. |
...
curl -sk https://localhost:8823/api/secrets

# Get specific secret
curl -sk https://localhost:8823/api/secrets/external_services.openai.api_key

# Get schema structure (all values redacted)
curl -sk https://localhost:8823/api/secrets/inspect

# Update secret (re-encrypts)
curl -sk -X POST -H 'Content-Type: application/json' \
  -d '{"value":"new-value"}' \
  https://localhost:8823/api/secrets/external_services.openai.api_key
```
