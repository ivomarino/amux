...
Most of this composes with primitives amux already has: worker logs + `?plain=1` clean export, the Gmail email API, the watchdog/self-healing + crash-dump, the amux Cloud gateway, Litestream→R2 backup, and the shared board.

---

## A. Support bundle & log export  *(anchor / quick win)*

1. **Export support bundle** — one click on a Support tab zips: recent `server.log`, ANSI-stripped worker logs (reuse `?plain=1`), redacted config/env, version + git commit, a system snapshot (OS, disk, mem, uptime), active workers, and recent errors. Download or send.
...
4. **Per-item share** — on the logs/peek tab, "Share this worker's log with support" and "Copy diagnostics" (version + env summary + health) for quick paste.
...
9. **Heartbeat to amux Cloud** — periodic anonymized health POST (version, worker/error counts, restart count; no logs, no PHI). Customer toggles it on. Lets us spot trouble before they report it.
...
16. **Request support worker** — customer clicks to mint a time-boxed, scoped, expiring token that gives us a read-only (or approval-gated) view of their dashboard, relayed through the amux Cloud gateway so no inbound ports are needed. Auto-expires; every action is audit-logged and visible to them.
17. **Support-scoped peek** — share one worker's live output with us behind a scoped, redacted, expiring link.
18. **Support-to-customer messaging** — push a dashboard banner ("maintenance tonight", "fix deployed"), or with consent send a message into a worker.
