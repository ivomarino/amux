...
**Goal (Ethan, 2026-08-02):** a session on any provider has the same capabilities inside amux as a Claude session. This file is the criteria list and the living scorecard. A capability is MET only with evidence (the ethos rule: verified, not assumed). Re-score when a provider's CLI changes.
...
| 3 | amux CLI + board from inside the session | board writes, sends, whoami work | MET | MET | SE-2 status-update posted by the Gemini lane itself |
| 4 | Memory injection | amux-composed memory reaches the model at launch | MET | MET (this commit) | Session-scoped GEMINI.md mirrored into `--include-directories`; no repo GEMINI.md touched |
| 5 | Launch parity (flags, yolo, model, resume meta) | provider flags normalized, session id persisted | MET | MET | start_session gemini branch (`--yolo`, `--skip-trust`, `--model auto`, gemini_session_id) |
...
| 9 | Token/cost tracking | per-session tokens + $ in Cost tab | MET | **GAP** | Ledger reads Claude JSONL only; Gemini lanes invisible to Cost — card AMUX-2230 |
...
| 15 | Guards (commit/push/staged), tags, peek, steering composer, schedules, archive | provider-agnostic mechanics | MET | MET | tmux/git/DB-based; no provider branch exists |
