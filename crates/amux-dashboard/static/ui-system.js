...
(function installAmuxBasecoatUI() {
  ...
  let dialogBackdropProbeTimer = 0;
  let dialogBackdropFailureSent = false;
  function checkHiddenDialogBackdrops() {
    if (dialogBackdropFailureSent) return;
    if (dialogBackdropProbeTimer) clearTimeout(dialogBackdropProbeTimer);
    // Legacy overlays transition their background for 250ms after close. Probe
    // after the MOST RECENT mutation plus that window: an already-scheduled
    // open probe must not inspect a dialog halfway through its later close.
    dialogBackdropProbeTimer = setTimeout(() => {
      dialogBackdropProbeTimer = 0;
      const hidden = [...document.querySelectorAll("[data-ui-dialog][aria-hidden='true']")];
      const painted = hidden.filter(dialog => {
        const style = getComputedStyle(dialog);
        if (style.display === 'none' || style.visibility === 'hidden' || Number(style.opacity || 1) === 0) return false;
        const background = style.backgroundColor || '';
        const alpha = background.match(/rgba?\([^)]*[, /]([\d.]+)\s*\)$/);
        const hasBackground = background !== 'transparent' && (!alpha || Number(alpha[1]) > 0.01);
        const backdrop = style.backdropFilter || style.webkitBackdropFilter || 'none';
        return hasBackground || backdrop !== 'none';
      });
      if (!painted.length) return;
      dialogBackdropFailureSent = true;
      const payload = {
        kind: 'ui-dialog-backdrop',
        verdict: 'hidden-dialog-painted',
        measured: true,
        n_considered: hidden.length,
        dialogs: painted.slice(0, 8).map(dialog => dialog.id || dialog.className || dialog.tagName),
      };
      console.warn('amux: hidden dialog painted a backdrop', payload);
      try {
        if (typeof window._clientDebug === 'function') window._clientDebug(payload.kind, payload);
        else if (navigator.sendBeacon) navigator.sendBeacon('/api/client-debug',
