...
fn worker_card_and_peek_share_actions_and_the_canonical_file_entry() {
    ...
    assert!(
        browse.contains("openDirectoryExplorer(root, name)"),
        "worker file entry must use the canonical Directory Explorer"
    );
    assert!(
        browse.contains("verdict: 'canonical-files-route'"),
        "worker file entry must log the canonical-route verdict"
    );
...
#[test]
fn basecoat_and_the_amux_ui_system_ship_as_one_offline_shell() {
    let html = asset("index.html");
    let sw = asset("sw.js");
    let basecoat_css = asset("vendor/basecoat/basecoat.min.css");
    let basecoat_js = asset("vendor/basecoat/all.min.js");
    let bridge = asset("ui-system.js");
    let system_css = asset("ui-system.css");

    assert!(basecoat_css.len() > 150_000, "vendored Basecoat CSS is missing or truncated");
    assert!(basecoat_js.len() > 30_000, "vendored Basecoat runtime is missing or truncated");
    assert!(system_css.len() > 10_000, "AMUX Basecoat layer is missing or truncated");
    assert!(bridge.contains("window.basecoat.initAll()"), "dynamic DOM must initialize Basecoat");
    assert!(bridge.contains("new MutationObserver(scheduleEnhance)"), "dynamic renderers need semantic enhancement");
    assert!(
        system_css.contains("[data-ui-dialog][aria-hidden='false']"),
        "Basecoat backdrops must style only open dialogs; a hidden board-edit overlay once blurred the whole SPA"
    );
    assert!(
        bridge.contains("verdict: 'hidden-dialog-painted'")
            && bridge.contains("kind: 'ui-dialog-backdrop'")
            && bridge.contains("clearTimeout(dialogBackdropProbeTimer)")
            && bridge.contains("}, 350);")
            && bridge.contains("navigator.sendBeacon('/api/client-debug'"),
        "a hidden dialog that paints past the close transition must announce itself in amux logs"
    );

    let base_css = html.find("href=\"/vendor/basecoat/basecoat.min.css\"").expect("Basecoat CSS link");
    let app_css = html.find("href=\"/app.css\"").expect("legacy app CSS link");
    let ui_css = html.find("href=\"/ui-system.css\"").expect("AMUX UI-system CSS link");
    let base_js = html.find("<script src=\"/vendor/basecoat/all.min.js\"").expect("Basecoat runtime script");
    let ui_js = html.find("<script src=\"/ui-system.js\"").expect("AMUX UI bridge script");
    let app_js = html.find("<script src=\"/app.js\"").expect("AMUX app script");
    assert!(base_css < app_css && app_css < ui_css, "CSS order must be Basecoat → legacy → AMUX system");
    assert!(base_js < ui_js && ui_js < app_js, "JS order must be Basecoat → bridge → AMUX app");
    ...
    for url in [
        ...
        assert!(sw.contains(url), "offline shell omitted `{url}`");
    }
}

#[test]
fn every_declared_page_is_covered_by_the_basecoat_bridge() {
    let html = asset("index.html");
    let app = asset("app.js");
    let bridge = asset("ui-system.js");
    let mut pages = Vec::new();
    for tail in html.split("id=\"").skip(1) {
        let Some(end) = tail.find('"') else { continue };
        let id = &tail[..end];
        if id.ends_with("-view") && !pages.iter().any(|p| p == id) {
            pages.push(id.to_string());
        }
    }
    assert!(pages.len() >= 25, "page inventory unexpectedly shrank to {} views", pages.len());
    for id in &pages {
        assert!(bridge.contains(&format!("'{id}'")), "Basecoat bridge omitted page `{id}`");
    }
    // Trends is an intentional nested mode inside Messages. Workspace/Grid has
    // its own enter/exit controller because it can embed the other pages. Every
    // other page must be dispatchable, including Graph/Journal/Habits which are
    // currently reached through embeds rather than primary-tab buttons.
    assert!(app.contains("function enterGridMode("), "Workspace/Grid lost its dedicated controller");
    for id in pages.iter().filter(|id| !matches!(id.as_str(), "trends-view" | "grid-view")) {
        let dom = id.trim_end_matches("-view");
        let route = if dom == "session" { "sessions" } else { dom };
        let tuple = format!("['{dom}', '{route}',");
        assert!(app.contains(&tuple), "switchView omitted declared page `{id}`");
    }
    for mapping in [
        "mdai:'mdai-view'",
        "email:'email-view'",
        ...
        assert!(app.contains(mapping), "workspace embed allowlist omitted `{mapping}`");
    }
    assert!(
        app.contains("catch(e) { setTimeout(_applyEmbedView, 50); return; }")
            && app.find("_embedViewApplied = true").unwrap()
                > app.find("switchView(window._embedView)").unwrap(),
        "workspace embeds must retry until the view dispatcher is initialized"
    );
    for needle in [
        "role', 'tabpanel'",
        "role', 'tablist'",
        ...
        assert!(bridge.contains(needle), "semantic UI bridge lost `{needle}`");
    }
}

#[test]
fn cross_context_capabilities_use_canonical_components() {
    let app = asset("app.js");
    let html = asset("index.html");

    for helper in [
        "function _directoryExplorerBreadcrumb(",
        "async function _directoryExplorerFetch(",
        "function _directoryExplorerBuildRow(",
        "async function _directoryExplorerToggleExpand(",
        "function _directoryExplorerRender(",
        "function _createTerminalSurface(",
    ] {
        assert_eq!(app.matches(helper).count(), 1, "canonical helper `{helper}` must have one definition");
    }
    for needle in [
        "_directoryExplorerRender(body, dirPath, data, {",
        "const entries = _directoryExplorerRender(body, path, data, options)",
        "return openFilePreview(filePath)",
        "_renderBoardColumnsInto(document.getElementById('peek-issues-cols')",
        "_renderBoardColumnsInto(container, visible",
        "_cmdHistItemHTML(e, _msgCtxHistory())",
        "_cmdHistItemHTML(e, _msgCtxPeek())",
        "_cmdHistItemHTML(m, _msgCtxMessages())",
        "renderScheduler({",
        "const surface = _createTerminalSurface(container)",
        "const surface = _createTerminalSurface(container, {",
    ] {
        assert!(app.contains(needle), "a cross-context surface stopped using `{needle}`");
    }
    for marker in [
        "data-component=\"directory-explorer\"",
        "data-component=\"message-timeline\"",
        "data-component=\"scheduler-surface\"",
        "data-component=\"terminal-surface\"",
        "data-component=\"worker-session-console\"",
    ] {
        assert!(html.contains(marker), "static component contract omitted `{marker}`");
    }
    for obsolete in [
        "function _renderExploreEntries(",
        "function loadExplore(",
        "id=\"explore-overlay\"",
    ] {
        assert!(
            !app.contains(obsolete) && !html.contains(obsolete),
            "obsolete parallel Directory Explorer returned as `{obsolete}`"
        );
    }
}

#[test]
fn calendar_surface_does_not_wait_for_board_network_before_rendering() {
    let app = asset("app.js");
    let css = asset("app.css");
    let branch = app
        .split("if (view === 'calendar') {")
        .nth(1)
        .expect("Calendar view branch exists");
    let ensure = branch.find("_fcEnsureInit();").expect("Calendar starts its UI immediately");
    let fetch = branch.find("fetchBoard().then").expect("Calendar still refreshes board data");
    assert!(
        ensure < fetch,
        "Calendar must initialize before the board request; offline/slow board data cannot blank the page"
    );
    for needle in [
        "function _fcEnsureInit(attempt = 0)",
        "if (window.FullCalendar)",
        "if (attempt < 20) setTimeout",
    ] {
        assert!(app.contains(needle), "Calendar loader omitted `{needle}`");
    }
    assert!(
        css.contains("#calendar-view { flex: 1 1 auto; min-height: calc(100dvh - 112px); }"),
        "Calendar needs a non-zero layout surface while its renderer initializes"
    );
}

#[test]
fn static_dom_ids_are_unique() {
    let html = asset("index.html");
    let mut seen = std::collections::HashSet::new();
    for tail in html.split("id=\"").skip(1) {
        let Some(end) = tail.find('"') else { continue };
        let id = &tail[..end];
        assert!(seen.insert(id), "duplicate static DOM id `{id}` makes component targeting ambiguous");
    }
    assert!(seen.len() > 400, "static DOM inventory unexpectedly shrank to {} ids", seen.len());
}
