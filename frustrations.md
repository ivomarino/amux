...
FIX: not fixed. The decompose nudge should re-evaluate the card's CURRENT shape before firing (type gates satisfiable, title/desc no longer a raw "**Prompt:**" capture), or clear the auto-captured origin marker when a card is retitled/retyped away from its capture form. A nudge that fires on origin rather than present state cannot tell a raw capture from a card someone already fixed, so it nags the fixed one forever.
