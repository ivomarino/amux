...
FIX: 2eceea7 documents `/api/debug/invariants` in the CLAUDE.md observability table and states plainly that a PASS is invisible on the health endpoint. That is the cheap half. The durable half is unfixed: the health endpoint could accept `?id=<invariant_id>` or return the id list behind the counts, so confirming a specific check ran does not require knowing which of two endpoints to reach for. Same shape as the rule this file exists for -- an empty result read as evidence, where a positive was never expressible.
