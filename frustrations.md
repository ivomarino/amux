...
## amux-launched browser dies on every server self-adopt, and the death is silent
AREA: browser
SEVERITY: slows
STATUS: open
DATE: 2026-08-15
SESSION: amux
CARD: AMUX-3184
SYMPTOM: Driving the dashboard for the ollama UI E2E, the amux-launched Chrome (POST /api/browser/start, a Playwright/CDP child of the server) vanished twice mid-test. Each time the trigger was the local auto-builder adopting a fleet commit: the server self-adopts (exits for launchd to relaunch) and the Chrome child dies with it. The worse half is the instrument. GET /api/browser/screenshot returned {"path": null} with NO error the first time, a successful-looking call that produced nothing, and only the NEXT action verb said "no amux-launched browser is running". So a dead browser reads as a blank screenshot rather than a clear "the session ended, restart it".
COST: ~6 minutes across two restarts, plus the peek terminal had to be re-opened each time (openPeek eval) and the tmux pane re-rendered its shell setup instead of the codex TUI, so the worker's response had to be read from the peek history API instead of the screen. On a shared checkout where ANY session's commit swaps the binary every ~60s, any browser-driven task longer than a build cycle hits this.
FIX: Not fixed (needs browser-lifecycle work, not a workaround). Two directions: (a) the cheap instrument half, /api/browser/screenshot and /api/browser/action should return a clear actionable error when the session is gone ("browser ended, server restarted at build <hash>; POST /api/browser/start") instead of {"path": null}, so a dead browser never reads as an empty-but-ok result (ethos rule 4); (b) the durable half, launch Chrome DETACHED (not a server child) and persist its cdp_http/cdp_port/pid (the start response already returns all three), so a freshly self-adopted server re-attaches to the still-alive Chrome instead of orphaning it. (a) is small and stops the silent-blank misread today; (b) is what makes a browser task survive the constant builder swaps this checkout produces.
